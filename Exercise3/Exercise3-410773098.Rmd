...
```{r ans12}
classData[(classData$`公-10-06` == "有看公告"),1]
```
...
```{r ans13}
x <-classData$學號[(classData$`公-10-06` == "有看公告" & 
    classData$`練2018-10-17-1` == "1")]
y <-classData$學號[(classData$`公-10-06` == "有看公告")]
a <-length(x)
b <-length(y)
a / b
```
...
```{r ans14}
x <-classData$學號[(classData$`公-10-06` == "沒看公告")]
y <-classData$學號[(classData$`公-10-06` == "沒看公告" & 
    classData$`練2018-10-17-1` == "1")]
a <-length(x)
b <-length(y)
b / a
...
```{r ans21}
library(lubridate)
x <-mdy_hms(hwData$Timestamp)
as.POSIXct(x , tz = "Asia/Taipei")
```
...
```{r ans22}
library(lubridate)
a <-ymd_hms("2018-10-24 00:00:00")
hwData$deadline <-as.POSIXct(a)
```
...
```{r ans23}
library(lubridate)
a <-as.POSIXct(hwData$deadline , tz = "Asia/Taipei")
x <-mdy_hms(hwData$Timestamp)
b <-as.POSIXct(x , tz = "Asia/Taipei")
hwData$early <-difftime(a , b , units = "days")
```
...
```{r ans25}
earlyDays <-hwData$early[hwData$作業代碼 == "hw1"]
table(earlyDays)
```
